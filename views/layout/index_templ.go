...
func Base(title string) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, templ_7745c5c3_W io.Writer) (templ_7745c5c3_Err error) {
		...
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("</div><div class=\"py-8 group flex h-8 items-center hover:bg-terminal-hover\"><div class=\"bg-primary-400 w-[2px] h-[30px] shrink-0 animate-blink\"></div><form hx-post=\"/commands\" hx-indicator=\"#spinner\" hx-target=\"#commandDisplay\" hx-swap=\"innerHTML transition:true\" hx-on::after-request=\"this.reset()\"><input name=\"command\" id=\"commandBox\" type=\"text\" autocomplete=\"off\" placeholder=\"$ enter command here\" class=\"px-4 border-none focus:ring-transparent text-white leading-tight bg-transparent\"></form></div><footer class=\"bg-primary flex flex-row justify-between items-center\"><div class=\"flex flex-row space-x-4\"><span id=\"currentText\" hx-swap-oob=\"true\" class=\"bg-secondary-500 text-white px-2 text-sm\">home.txt</span> <span id=\"spinner\" class=\"loading loading-infinity loading-sm htmx-indicator\"></span></div><div class=\"flex flex-row space-x-4\"><span id=\"tabSize\" class=\"text-white px-2 text-sm hover:bg-primary-400 cursor-pointer\">Ln 81, Col 122</span> <span class=\"text-white px-2 text-sm hover:bg-primary-400 cursor-pointer\">htmx</span> <span class=\"text-white px-2 text-sm hover:bg-primary-400 cursor-pointer\">templ</span> <span id=\"tabSize\" class=\"text-white px-2 text-sm hover:bg-primary-400 cursor-pointer\">Tab Size: 4</span> <span id=\"errorText\" class=\"bg-error-base text-white px-2 text-sm\">Errors</span></div></footer></layout><script lang=\"js\">\n\t\t\t     document.body.addEventListener('htmx:beforeSwap', function(evt) {\n\t\t\t    if(evt.detail.xhr.status === 404){\n\t\t\t        // alert the user when a 404 occurs (maybe use a nicer mechanism than alert())\n\t\t\t        alert(\"Error: Could Not Find Resource\");\n\t\t\t    } else if(evt.detail.xhr.status === 422){\n\t\t\t        // allow 422 responses to swap as we are using this as a signal that\n\t\t\t        // a form was submitted with bad data and want to rerender with the\n\t\t\t        // errors\n\t\t\t        //\n\t\t\t        // set isError to false to avoid error logging in console\n                    alert(\"Bad Request Debug\")\n\t\t\t        evt.detail.shouldSwap = true;\n\t\t\t        evt.detail.isError = false;\n\t\t\t    } else if(evt.detail.xhr.status === 418){\n\t\t\t        // if the response code 418 (I'm a teapot) is returned, retarget the\n\t\t\t        // content of the response to the element with the id `teapot`s\n\t\t\t        evt.detail.shouldSwap = true;\n\t\t\t        evt.detail.target = htmx.find(\"#teapot\");\n\t\t\t    }\n\t\t\t});\n\t\t\t</script></body></html>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		if !templ_7745c5c3_IsBuffer {
