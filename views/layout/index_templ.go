...
func Base(title string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("</main><div class=\"py-4 flex items-center bg-terminal-base hover:bg-terminal-hover px-4\"><div class=\"bg-primary-400 w-[2px] h-[30px] shrink-0 animate-blink\"></div><form hx-post=\"/commands\" hx-indicator=\"#spinner\" hx-target=\"#commandDisplay\" hx-swap=\"innerHTML transition:true\" hx-on::after-request=\"this.reset()\" class=\"flex-grow\"><input name=\"command\" id=\"commandBox\" type=\"text\" autocomplete=\"off\" placeholder=\"$ enter command here\" class=\"w-full px-4 border-none focus:ring-transparent text-white leading-tight bg-transparent\"></form></div><footer class=\"flex md:flex-row justify-between items-center p-4 rounded-lg\"><div class=\"flex md:flex-row sm:space-y-0 md:space-x-4\"><span id=\"currentText\" class=\"bg-secondary-500 text-white px-2 text-lg rounded-r-md\">home.txt</span> <span id=\"spinner\" class=\"loading loading-infinity loading-sm htmx-indicator\"></span></div><div class=\"flex md:flex-row sm:space-y-0 md:space-x-4\"><span id=\"tabSize\" class=\"text-white px-2 text-lg hover:bg-primary cursor-pointer\">Ln 81, Col 122</span> <span class=\"hidden md:block text-white px-2 text-lg hover:bg-primary cursor-pointer tooltip\" data-tip=\"htmx\">htmx</span> <span class=\"hidden md:block text-white px-2 text-lg hover:bg-primary cursor-pointer tooltip\">templ</span> <span id=\"tabSize\" class=\"hidden md:block text-white px-2 text-lg hover:bg-primary-400 cursor-pointer\">Tab Size: 4</span> <span id=\"errorText\" class=\"bg-error-base text-white px-2 text-lg rounded-l-md\">nvim</span></div></footer></div><script lang=\"js\">\n\t\t\t     document.body.addEventListener('htmx:beforeSwap', function(evt) {\n\t\t\t        if(evt.detail.xhr.status === 404){\n\t\t\t            alert(\"Error: Could Not Find Resource\");\n\t\t\t        } else if(evt.detail.xhr.status === 422){\n\t\t\t            alert(\"Bad Request Debug\")\n\t\t\t            evt.detail.shouldSwap = true;\n\t\t\t            evt.detail.isError = false;\n\t\t\t        } else if(evt.detail.xhr.status === 418){\n\t\t\t            evt.detail.shouldSwap = true;\n\t\t\t            evt.detail.target = htmx.find(\"#teapot\");\n\t\t\t        }\n\t\t\t    });\n\t\t\t</script></body></html>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return templ_7745c5c3_Err
